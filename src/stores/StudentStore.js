...
class StudentStore {
  ...
  getStudentCourseFromWebServer(){
    return axios.get(`/course/student/${this.userId}`).catch(err=>{
      console.log(err);
    });
  }
  getStudentHistoryCourseFromWebServer(){
    return axios.get(`/course/student/${this.userId}/history`)
  }

  deleteClass(classId){
    return axios.delete(`/choice/${classId}/${this.userId}`).then(err=>{
      console.log(err);
    });
  }
